...
def nettcr_one_chain():
    cdr_in = Input(shape=(40,20))
    pep_in = Input(shape=(50,20))
    ...
    output1 = GlobalMaxPooling1D()(layer(pep_conv1, cdr_conv1))
    output2 = GlobalMaxPooling1D()(layer(cdr_conv1, pep_conv1))
    pep_conv3 = Conv1D(16, 3, padding='same', activation='sigmoid', kernel_initializer='glorot_normal')(pep_in)
    ...
    output3 = GlobalMaxPooling1D()(layer(pep_conv3, cdr_conv3))
    output4 = GlobalMaxPooling1D()(layer(cdr_conv3, pep_conv3))
    pep_conv5 = Conv1D(16, 5, padding='same', activation='sigmoid', kernel_initializer='glorot_normal')(pep_in)
    ...
    output5 = GlobalMaxPooling1D()(layer(pep_conv5, cdr_conv5))
    output6 = GlobalMaxPooling1D()(layer(cdr_conv5, pep_conv5))
    pep_conv7 = Conv1D(16, 7, padding='same', activation='sigmoid', kernel_initializer='glorot_normal')(pep_in)
    ...
    output7 = GlobalMaxPooling1D()(layer(pep_conv7, cdr_conv7))
    output8 = GlobalMaxPooling1D()(layer(cdr_conv7, pep_conv7))
    pep_conv9 = Conv1D(16, 9, padding='same', activation='sigmoid', kernel_initializer='glorot_normal')(pep_in)
    ...
    output9 = GlobalMaxPooling1D()(layer(pep_conv9, cdr_conv9))
    output10 = GlobalMaxPooling1D()(layer(cdr_conv9, pep_conv9))
    
    pep_cat = concatenate([pep_pool1, pep_pool3, pep_pool5, pep_pool7, pep_pool9])
    cdr_cat = concatenate([cdr_pool1, cdr_pool3, cdr_pool5, cdr_pool7, cdr_pool9])
    pep_cdr_cat = concatenate([output1, output3, output5, output7, output9])
    cdr_pep_cat = concatenate([output2, output4, output6, output8, output10])

    cat = concatenate([pep_cat, cdr_cat, pep_cdr_cat, cdr_pep_cat], axis=1)
