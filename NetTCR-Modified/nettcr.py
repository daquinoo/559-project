...
if chain=='ab':
    ...
    pep_train = utils.enc_list_bl_max_len(train_data.peptide, encoding, 50)
    tcrb_train = utils.enc_list_bl_max_len(train_data.CDR3b, encoding, 40)
    y_train = np.array(train_data.binder)
...
pred_df['binder_prediction'] = (pred_df['prediction'] >= 0.5).astype(int)
with open('results.txt', "w") as file:
    file.write(utils.get_performance(pred_df['prediction'], pred_df['binder'], pred_df['binder_prediction']))
mdl.save('model.h5')
