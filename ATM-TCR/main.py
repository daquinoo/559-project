...
def main():
    ...
    # Read data
    x_pep_train, x_tcr_train, y_train = data_io_tf.read_pTCR(args.infile)
    x_pep_test, x_tcr_test, y_test = data_io_tf.read_pTCR(args.testfile)

    # Define dataloader
    train_loader = define_dataloader(x_pep_train, x_tcr_train, y_train,
                                     args.max_len_pep, args.max_len_tcr,
                                     padding=args.padding,
                                     batch_size=args.batch_size, device=device)
    test_loader = define_dataloader(x_pep_test, x_tcr_test, y_test,
    ...
    if args.mode == 'train':
        ...
        for epoch in range(1, args.epoch + 1):
            ...
            perf_test, loss = get_performance_batchiter(
        ...
        if args.save_model:
            with open('result/perf_' + os.path.splitext(os.path.basename(args.model_name))[0] + '.txt', "w") as file:
                file.write(perf_test)
        ...
        print('[INDEP] {} ----------------')
        perf_indep, loss = get_performance_batchiter(
