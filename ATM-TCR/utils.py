...
def get_performance(binding_score, binding):
    binding_pred = [round(s[0]) for s in binding_score]
    accuracy = str(accuracy_score(binding, binding_pred))
    precision = str(precision_score(binding, binding_pred))
    recall = str(recall_score(binding, binding_pred))
    f1 = str(f1_score(binding, binding_pred))
    auc = str(roc_auc_score(np.array(binding), np.array(binding_score)) if len(np.unique(np.array(binding))) != 1 else -1)
